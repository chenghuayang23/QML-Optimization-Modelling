...
# firing cost is changed from 0 to 8000, the step is 1000
firingCost = 8000    # euro
...
if model.status == GRB.Status.OPTIMAL: # If optimal solution is found
    # print total cost and hholding cost for each type of product
    print('%d, %10.2f, %10.2f, %10.2f, %10.2f' % (firingCost,
                                                model.objVal,    
                                                sum(r[0,k].x * holdingCosts[0] for k in K),
                                                sum(r[1,k].x * holdingCosts[1] for k in K),
                                                sum(r[2,k].x * holdingCosts[2] for k in K)))
